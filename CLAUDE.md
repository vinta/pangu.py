...
- Commented-out asserts, FIXME comments, and `xfail(strict=True)`/`skip` markers in `tests/core/` are intentional 1:1 ports of upstream FIXME cases and `it.todo`/`describe.todo` — leave them. The per-file test lint ignores in `pyproject.toml` exist for the same reason; don't widen them casually.
