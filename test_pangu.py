...
class SpacingTest(PanguTest):
    ...
    def test_skip_underscore(self):
        ...
        self.assertEqual(pangu.spacing('Vinta_Mollie'), u'Vinta_Mollie')
        self.assertEqual(pangu.spacing('Vinta _ Mollie'), u'Vinta _ Mollie')

    # 兩邊都加空格

    def test_alphabets(self):
        self.assertEqual(pangu.spacing('中文abc'), u'中文 abc')
        self.assertEqual(pangu.spacing('abc中文'), u'abc 中文')

    def test_numbers(self):
        self.assertEqual(pangu.spacing('中文123'), u'中文 123')
        self.assertEqual(pangu.spacing('123中文'), u'123 中文')
    ...
    def test_greek_and_coptic(self):
        self.assertEqual(pangu.spacing('中文β漢字'), u'中文 β 漢字')
        self.assertEqual(pangu.spacing('中文 β 漢字'), u'中文 β 漢字')
        self.assertEqual(pangu.spacing('我是α，我是Ω'), u'我是 α，我是 Ω')
    ...
    def test_minus(self):
        ...
        self.assertEqual(pangu.spacing('陳上進-Vinta'), u'陳上進 - Vinta')
        self.assertEqual(pangu.spacing('得到一個A-B的結果'), u'得到一個 A-B 的結果')
        self.assertEqual(pangu.spacing('长者的智慧和复杂的维斯特洛- 文章'), u'长者的智慧和复杂的维斯特洛 - 文章')
    ...
    def test_slash(self):
        ...
        self.assertEqual(pangu.spacing('Vinta/陳上進'), u'Vinta / 陳上進')
        self.assertEqual(pangu.spacing('陳上進/Vinta'), u'陳上進 / Vinta')
        self.assertEqual(pangu.spacing('Mollie/陳上進/Vinta'), u'Mollie / 陳上進 / Vinta')

        self.assertEqual(pangu.spacing('得到一個A/B的結果'), u'得到一個 A/B 的結果')
        self.assertEqual(pangu.spacing('2016-12-26(奇幻电影节) / 2017-01-20(美国) / 詹姆斯麦卡沃伊'), u'2016-12-26 (奇幻电影节) / 2017-01-20 (美国) / 詹姆斯麦卡沃伊')
        self.assertEqual(pangu.spacing('/home/和/root是Linux中的頂級目錄'), u'/home/ 和 /root 是 Linux 中的頂級目錄')
        self.assertEqual(pangu.spacing('當你用cat和od指令查看/dev/random和/dev/urandom的內容時'), u'當你用 cat 和 od 指令查看 /dev/random 和 /dev/urandom 的內容時')
    ...
    def test_at(self):
        ...
        self.assertEqual(pangu.spacing('請@vinta吃大便'), u'請 @vinta 吃大便')
        self.assertEqual(pangu.spacing('請@陳上進 吃大便'), u'請 @陳上進 吃大便')

    def test_hash(self):
        self.assertEqual(pangu.spacing('前面#後面'), u'前面 #後面')
        self.assertEqual(pangu.spacing('前面C#後面'), u'前面 C# 後面')
    ...
    def test_colon(self):
        ...
        self.assertEqual(pangu.spacing('前面: 後面'), u'前面：後面')
        self.assertEqual(pangu.spacing('前面 :後面'), u'前面：後面')
        self.assertEqual(pangu.spacing('電話:123456789'), u'電話：123456789')
        self.assertEqual(pangu.spacing('前面:)後面'), u'前面：) 後面')
        self.assertEqual(pangu.spacing('前面:I have no idea後面'), u'前面：I have no idea 後面')
        self.assertEqual(pangu.spacing('前面: I have no idea後面'), u'前面: I have no idea 後面')
    ...
    def test_comma(self):
        ...
        self.assertEqual(pangu.spacing('前面, 後面'), u'前面，後面')
        self.assertEqual(pangu.spacing('前面 ,後面'), u'前面，後面')
        self.assertEqual(pangu.spacing('前面,'), u'前面，')
        self.assertEqual(pangu.spacing('前面, '), u'前面，')
    ...
    def test_period(self):
        ...
        self.assertEqual(pangu.spacing('前面. 後面'), u'前面。後面')
        self.assertEqual(pangu.spacing('前面 .後面'), u'前面。後面')
        self.assertEqual(pangu.spacing('黑人問號.jpg 後面'), u'黑人問號.jpg 後面')
    ...
    def test_u00b7(self):
        self.assertEqual(pangu.spacing('前面·後面'), u'前面・後面')
        self.assertEqual(pangu.spacing('喬治·R·R·馬丁'), u'喬治・R・R・馬丁')
        self.assertEqual(pangu.spacing('M·奈特·沙马兰'), u'M・奈特・沙马兰')
    ...
    def test_parentheses(self):
        ...
        self.assertEqual(pangu.spacing('前面(中文123)後面'), u'前面 (中文 123) 後面')
        self.assertEqual(pangu.spacing('前面(123漢字)後面'), u'前面 (123 漢字) 後面')
        self.assertEqual(pangu.spacing('前面(中文123) tail'), u'前面 (中文 123) tail')
        self.assertEqual(pangu.spacing('head (中文123漢字)後面'), u'head (中文 123 漢字) 後面')
        self.assertEqual(pangu.spacing('head (中文123漢字) tail'), u'head (中文 123 漢字) tail')
        self.assertEqual(pangu.spacing('(or simply "React")'), u'(or simply "React")')
        self.assertEqual(pangu.spacing("OperationalError: (2006, 'MySQL server has gone away')"), u"OperationalError: (2006, 'MySQL server has gone away')")
        self.assertEqual(pangu.spacing('我看过的电影(1404)'), u'我看过的电影 (1404)')
        self.assertEqual(pangu.spacing('Chang Stream(变更记录流)是指collection(数据库集合)的变更事件流'), u'Chang Stream (变更记录流) 是指 collection (数据库集合) 的变更事件流')
    ...
    def test_back_quote_back_quote(self):
        self.assertEqual(pangu.spacing('前面`中間`後面'), u'前面 `中間` 後面')
    ...
    def test_single_quote_single_quote(self):
        self.assertEqual(pangu.spacing("Why are Python's 'private' methods not actually private?"), u"Why are Python's 'private' methods not actually private?")
        self.assertEqual(pangu.spacing("陳上進 likes 林依諾's status."), u"陳上進 likes 林依諾's status.")
        self.assertEqual(pangu.spacing("举个栗子，如果一道题只包含'A' ~ 'Z'意味着字符集大小是"), u"举个栗子，如果一道题只包含 'A' ~ 'Z' 意味着字符集大小是")
    ...
    def test_alphabets_u201c_u201d(self):
        self.assertEqual(pangu.spacing('阿里云开源“计算王牌”Blink，实时计算时代已来'), u'阿里云开源 “计算王牌” Blink，实时计算时代已来')
        self.assertEqual(pangu.spacing('苹果撤销Facebook“企业证书”后者股价一度短线走低'), u'苹果撤销 Facebook “企业证书” 后者股价一度短线走低')
        self.assertEqual(pangu.spacing('【UCG中字】“數毛社”DF的《戰神4》全新演示解析'), u'【UCG 中字】“數毛社” DF 的《戰神 4》全新演示解析')

    def test_parentheses_percent(self):
        self.assertEqual(pangu.spacing("丹寧控注意Levi's全館任2件25%OFF滿額再享85折！"), u"丹寧控注意 Levi's 全館任 2 件 25% OFF 滿額再享 85 折！")


class SpacingTextTest(PanguTest):

    def test_spacing_text(self):
        self.assertEqual(pangu.spacing_text('123'), u'123')
...
#         new_content = 1
#         expected_content = 2
#         self.assertEqual(pangu.spacing_file('123'), u'123')
